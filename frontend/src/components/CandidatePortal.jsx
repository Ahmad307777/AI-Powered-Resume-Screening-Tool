...
import {
  ...
  Sparkles, Loader2, ArrowLeft, User, Mail,
  Phone, MapPin, ChevronRight, Zap, Brain, Shield, Briefcase, Clock, Search
} from 'lucide-react';
import CVEnhancer from './CVEnhancer';
import { useNavigate } from 'react-router-dom';
import { useTheme, Logo, T } from '../theme.jsx';
...
function JobBoard({ onSelect }) {
  ...
  return (
    <div style={{ width:'100%', maxWidth:'960px', padding:'2rem 1.5rem', fontFamily:"'Outfit',sans-serif" }}>
      <style>{STYLES}</style>
      ...
      {/* Header */}
      <div style={{ marginBottom:'2rem', animation:'fadeUp .5s ease .05s both' }}>
        <div style={{ display:'inline-flex', alignItems:'center', gap:'.4rem', background:'rgba(255,136,0,.1)', border:'1px solid rgba(255,136,0,.25)', borderRadius:'999px', padding:'.3rem .9rem', color:T.orangeL, fontSize:'.75rem', fontWeight:700, letterSpacing:'.4px', marginBottom:'1rem' }}>
          <Zap size={12} color={T.orange}/> Open Positions
        </div>
        <h1 style={{ fontFamily:"'Outfit',sans-serif", fontSize:'clamp(1.8rem,3vw,2.5rem)', fontWeight:900, color:T.white, margin:'0 0 .5rem', letterSpacing:'-1px' }}>
          Find Your <span style={{ background:`linear-gradient(90deg,${T.orange},${T.orangeL})`, WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>Dream Role</span>
        </h1>
        <p style={{ color:T.grey, fontSize:'.95rem', margin:0 }}>
          {loading ? 'Loading positions…' : `${jobs.length} open position${jobs.length!==1?'s':''} — pick one to apply`}
        </p>
      </div>

      {/* Search */}
      {!loading && jobs.length > 0 && (
        <div style={{ display:'flex', alignItems:'center', gap:'.6rem', background:T.bgCard, border:`1px solid ${T.line}`, borderRadius:'14px', padding:'.65rem 1.1rem', marginBottom:'1.75rem', animation:'fadeUp .5s ease .1s both' }}
          ...
          <input type="text" placeholder="Search by role, skills, or recruiter…" value={search}
            onChange={e => setSearch(e.target.value)}
            style={{ flex:1, background:'transparent', border:'none', outline:'none', color:T.white, fontSize:'.95rem', fontFamily:"'Outfit',sans-serif" }}/>
          {search && (
            <button onClick={() => setSearch('')} style={{ background:'none', border:'none', cursor:'pointer', color:T.greyD, display:'flex', padding:0 }}>
              <XCircle size={15}/>
            </button>
          )}
        </div>
      ...
      {loading && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'1.25rem' }}>
          {[1,2,3].map(i => (
            <div key={i} style={{ height:'200px', borderRadius:'18px', background:T.bgCard, border:`1px solid ${T.line}`, overflow:'hidden', position:'relative' }}>
              <div style={{ position:'absolute', inset:0, background:'linear-gradient(90deg,transparent,rgba(255,136,0,.04),transparent)', backgroundSize:'200% 100%', animation:'shimmer 1.5s linear infinite' }}/>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Empty */}
      {!loading && jobs.length === 0 && (
        <div style={{ textAlign:'center', padding:'5rem 2rem', animation:'fadeIn .5s ease both' }}>
          <div style={{ fontSize:'3rem', marginBottom:'1rem' }}>📭</div>
          <h3 style={{ color:T.white, margin:'0 0 .5rem' }}>No Open Positions</h3>
          <p style={{ color:T.grey, fontSize:'.9rem' }}>No HR has posted a job yet. Check back soon.</p>
        </div>
      )}

      {/* No results */}
      {!loading && jobs.length > 0 && filtered.length === 0 && (
        <div style={{ textAlign:'center', padding:'3rem 2rem', animation:'fadeIn .3s ease both' }}>
          <div style={{ fontSize:'2rem', marginBottom:'.75rem' }}>🔍</div>
          <p style={{ color:T.grey }}>No positions match "<strong style={{ color:T.white }}>{search}</strong>"</p>
        </div>
      )}
      ...
      {!loading && filtered.length > 0 && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:'1.25rem' }}>
          {filtered.map((job, i) => {
            const isHov = hov === job.hr_id;
            return (
              <div key={job.hr_id}
                onMouseEnter={() => setHov(job.hr_id)}
                onMouseLeave={() => setHov(null)}
                onClick={() => onSelect(job)}
                style={{
                  background: isHov ? T.bgCard2 : T.bgCard,
                  border:`1px solid ${isHov ? T.lineO : T.line}`,
                  borderRadius:'18px', padding:'1.5rem',
                  display:'flex', flexDirection:'column', gap:'1rem',
                  cursor:'pointer',
                  transform: isHov ? 'translateY(-6px)' : 'none',
                  boxShadow: isHov ? '0 20px 50px rgba(0,0,0,.5),0 0 40px rgba(255,136,0,.08)' : '0 4px 20px rgba(0,0,0,.3)',
                  transition:'all .28s cubic-bezier(.4,0,.2,1)',
                  animation:`fadeUp .5s ease ${i*.07}s both`,
                  position:'relative', overflow:'hidden',
                }}>
                {isHov && <div style={{ position:'absolute', top:'-30px', right:'-30px', width:'120px', height:'120px', borderRadius:'50%', background:'radial-gradient(circle,rgba(255,136,0,.12) 0%,transparent 70%)', pointerEvents:'none' }}/>}

                <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'.75rem' }}>
                  <div style={{ width:'44px', height:'44px', borderRadius:'12px', background: isHov?'rgba(255,136,0,.18)':'rgba(255,136,0,.1)', border:'1px solid rgba(255,136,0,.25)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, transition:'background .25s' }}>
                    <Briefcase size={20} color={T.orange}/>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:'.3rem', background:'rgba(34,197,94,.08)', border:'1px solid rgba(34,197,94,.25)', borderRadius:'999px', padding:'.2rem .65rem', flexShrink:0 }}>
                    <div style={{ width:'6px', height:'6px', borderRadius:'50%', background:T.green, animation:'pulse 2s ease-in-out infinite' }}/>
                    <span style={{ color:T.green, fontSize:'.68rem', fontWeight:700 }}>Hiring</span>
                  </div>
                </div>

                <div>
                  <div style={{ color:T.white, fontWeight:800, fontSize:'1.1rem', letterSpacing:'-.3px', lineHeight:1.2 }}>{job.position}</div>
                  <div style={{ color:T.grey, fontSize:'.8rem', marginTop:'.3rem', display:'flex', alignItems:'center', gap:'.3rem' }}>
                    <User size={11} color={T.greyD}/> {job.hr_name}
                  </div>
                </div>

                <div style={{ display:'flex', alignItems:'center', gap:'.35rem', background:T.bgCard2, border:`1px solid ${T.line}`, borderRadius:'8px', padding:'.28rem .7rem', alignSelf:'flex-start' }}>
                  <Clock size={11} color={T.greyD}/>
                  <span style={{ color:T.grey, fontSize:'.75rem', fontWeight:600 }}>
                    {job.experience > 0 ? `${job.experience}+ yrs exp` : 'Any experience'}
                  </span>
                </div>
                ...
                {job.skills.length > 0 && (
                  <div style={{ display:'flex', flexWrap:'wrap', gap:'.35rem' }}>
                    {job.skills.slice(0,5).map(s => (
                      <span key={s} style={{ fontSize:'.68rem', fontWeight:600, padding:'.2rem .55rem', borderRadius:'6px', background:'rgba(255,136,0,.08)', border:'1px solid rgba(255,136,0,.2)', color:T.orangeL }}>{s}</span>
                    ))}
                    {job.skills.length > 5 && <span style={{ fontSize:'.68rem', color:T.greyD, padding:'.2rem .4rem' }}>+{job.skills.length-5} more</span>}
                  </div>
                )}

                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:'auto', paddingTop:'.5rem', borderTop:`1px solid ${T.line}` }}>
                  <span style={{ color: isHov?T.orange:T.greyD, fontSize:'.82rem', fontWeight:700, transition:'color .2s' }}>Apply Now</span>
                  <div style={{ width:'28px', height:'28px', borderRadius:'8px', background: isHov?'rgba(255,136,0,.15)':'rgba(255,255,255,.04)', border:`1px solid ${isHov?T.lineO:T.line}`, display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s' }}>
                    <ChevronRight size={14} color={isHov?T.orange:T.greyD}/>
                  </div>
                </div>
              </div>
            );
...
function AnimatedNumber({ target, suffix='%', duration=1200 }) {
  ...
  useEffect(() => {
    ...
    const steps = 60; const step = end/steps; let cur = 0;
    const t = setInterval(() => {
      cur = Math.min(cur+step, end); setVal(cur.toFixed(1));
...
function Particles() {
  const particles = Array.from({ length: 18 }, (_, i) => ({
    ...
    size: Math.random()*4+2, dur:`${Math.random()*4+3}s`, delay:`${Math.random()*4}s`,
...
function ProgressBar({ value, color, delay='0s' }) {
  ...
  return (
    <div style={{ height:'4px', background:'rgba(255,255,255,.06)', borderRadius:'99px', overflow:'hidden' }}>
      <div style={{ height:'100%', borderRadius:'99px', background:`linear-gradient(90deg,${c},${T.orangeL})`, '--target-width':`${value}%`, animation:`progressFill .9s ease ${delay} both`, width:`${value}%` }}/>
    </div>
  );
}
      ...
      {steps.map((s, i) => {
        ...
        return (
          <div key={s} style={{ display:'flex', alignItems:'center', gap:'.5rem' }}>
            <div style={{ width:current?'28px':'10px', height:'10px', borderRadius:'99px', background:done?T.orange:current?T.orange:'rgba(255,255,255,.1)', border:`1px solid ${current||done?T.orange:'rgba(255,255,255,.12)'}`, transition:'all .4s', boxShadow:current?'0 0 12px rgba(255,136,0,.6)':'none' }}/>
            {i < steps.length-1 && <div style={{ width:'32px', height:'1px', background:done?T.orange:'rgba(255,255,255,.08)', transition:'background .4s' }}/>}
          </div>
        );
      })}
    </div>
...
function HeroHeader({ activeConfig }) {
  const { T } = useTheme();
  const typed = useTypewriter(['Upload Your Resume','Get AI Feedback','Land Your Dream Job','Beat the Competition']);
  return (
    <div style={{ position:'relative', width:'100%', overflow:'hidden', borderRadius:'24px', marginBottom:'2rem', background:`linear-gradient(135deg,${T.bgCard} 0%,${T.bgCard2} 100%)`, border:`1px solid ${T.line}`, padding:'3rem 2.5rem' }}>
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        ...
        <h1 style={{ fontFamily:"'Outfit',sans-serif", fontSize:'clamp(1.8rem,4vw,2.8rem)', fontWeight:900, color:T.white, margin:'0 0 .5rem', letterSpacing:'-1.5px', lineHeight:1.1, animation:'fadeUp .6s ease .1s both' }}>
          <span style={{ background:`linear-gradient(90deg,${T.orange},${T.orangeL},#fff)`, backgroundSize:'200% auto', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', animation:'gradientShift 3s linear infinite' }}>{typed}</span>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:'.6rem', animation:'fadeUp .6s ease .3s both' }}>
          {[{icon:Brain,text:'AI Role Matching'},{icon:Zap,text:'Instant Results'},{icon:Shield,text:'Private & Secure'}].map(({icon:Icon,text}) => (
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'.4rem', animation:`fadeUp .5s ease ${delay} both` }}>
      ...
      <div onFocus={() => setFocus(true)} onBlur={() => setFocus(false)}
        style={{ position:'relative', transition:'all .2s', filter:focus?'drop-shadow(0 0 8px rgba(255,136,0,.2))':'none' }}>
        {children}
      </div>
    </div>
  );
      ...
      {result.success && (
        <>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'1rem', marginBottom:'2rem' }}>
            {[
              { value:result.data.confidenceScore?.toFixed(1)??'N/A', label:'AI Confidence', color:result.data.confidenceScore>=60?T.green:T.amber, delay:'.1s' },
              { value:result.data.keywordScore.toFixed(1),            label:'Keyword Match', color:T.orange,  delay:'.2s' },
              { value:result.data.semanticScore.toFixed(1),           label:'Semantic Sim.', color:T.orangeL, delay:'.3s' },
            ].map(ring => (
              <div key={ring.label} style={{ background:T.bgCard, border:`1px solid ${T.line}`, borderRadius:'18px', padding:'1.5rem', display:'flex', flexDirection:'column', alignItems:'center', gap:'.75rem', animation:`fadeUp .5s ease ${ring.delay} both` }}>
                <ScoreRing value={ring.value} label={ring.label} color={ring.color} delay={ring.delay}/>
              </div>
            ))}
            <div style={{ background:T.bgCard, border:`1px solid rgba(255,136,0,.2)`, borderRadius:'18px', padding:'1.5rem', display:'flex', flexDirection:'column', alignItems:'center', gap:'.75rem', animation:'fadeUp .5s ease .4s both' }}>
          ...
          {(result.data.experienceYears!==undefined||result.data.secondaryCategory) && (
            <div style={{ display:'flex', gap:'.6rem', flexWrap:'wrap', marginBottom:'1.5rem', animation:'fadeUp .5s ease .4s both' }}>
              ...
              {result.data.secondaryCategory && (
                <span style={{ display:'inline-flex', alignItems:'center', gap:'.35rem', padding:'.4rem .85rem', borderRadius:'999px', border:`1px solid rgba(255,136,0,.3)`, color:T.orangeL, background:'rgba(255,136,0,.07)', fontSize:'.8rem', fontWeight:600 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem', marginBottom:'1.5rem', animation:'fadeUp .5s ease .45s both' }}>
            {[
              { title:'✓ Matched Skills', color:T.green,  bg:'rgba(34,197,94,.08)',  bd:'rgba(34,197,94,.25)',  pBg:'rgba(34,197,94,.1)',  pBd:'rgba(34,197,94,.3)',  skills:result.data.matchedSkills },
              { title:'✗ Missing Skills', color:T.red,    bg:'rgba(239,68,68,.06)',  bd:'rgba(239,68,68,.2)',   pBg:'rgba(239,68,68,.1)',  pBd:'rgba(239,68,68,.3)',  skills:result.data.missingSkills },
            ].map(block => (
              <div key={block.title} style={{ background:block.bg, border:`1px solid ${block.bd}`, borderRadius:'14px', padding:'1.25rem' }}>
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:'.4rem' }}>
                  {block.skills.length>0
                    ? block.skills.map((s,i) => <span key={s} style={{ fontSize:'.72rem', fontWeight:700, padding:'.25rem .65rem', borderRadius:'6px', border:`1px solid ${block.pBd}`, background:block.pBg, color:block.color, animation:`fadeIn .3s ease ${i*.05}s both` }}>{s}</span>)
          ...
          <div style={{ background:'linear-gradient(135deg,rgba(255,136,0,.05),rgba(255,136,0,.02))', border:`1px solid rgba(255,136,0,.2)`, borderRadius:'16px', padding:'1.5rem', animation:'fadeUp .5s ease .5s both' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'.5rem', marginBottom:'.75rem' }}>
              <div style={{ width:'30px', height:'30px', borderRadius:'8px', background:'rgba(255,136,0,.12)', border:'1px solid rgba(255,136,0,.25)', display:'flex', alignItems:'center', justifyContent:'center' }}>
...
/* ═══════════════════════════════════════════════════════
   MAIN EXPORT
   ═══════════════════════════════════════════════════════ */
export default function CandidatePortal({ onApplicationSuccess }) {
  const navigate = useNavigate();
  const { T } = useTheme();
  ...
  // Show job board if no job selected yet
  if (!selectedJob) {
    return (
      <JobBoard
        onSelect={job => setSelectedJob({
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      fd.append('hr_id', jobConfig.hr_id || '');
  ...
  return (
    <div style={{ width:'100%', maxWidth:'900px', padding:'2rem 1.5rem', fontFamily:"'Outfit',sans-serif" }}>
      ...
      {!result && !loading && <HeroHeader activeConfig={jobConfig}/>}
        ...
        {loading ? (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'4rem 2rem', gap:'1.5rem', position:'relative', zIndex:1 }}>
            ...
            <div style={{ textAlign:'center' }}>
              ...
              <p style={{ color:T.grey, fontSize:'.9rem', lineHeight:1.6, maxWidth:'300px', margin:'0 auto' }}>Extracting text, comparing metrics, calculating cosine similarity…</p>
            </div>
            <div style={{ display:'flex', gap:'.5rem' }}>
              {[0,.15,.3].map(d => <div key={d} style={{ width:'8px', height:'8px', borderRadius:'50%', background:T.orange, animation:`pulse 1.2s ease-in-out ${d}s infinite` }}/>)}
            </div>
          </div>

        ) : result ? (
          <ResultView result={result} activeConfig={jobConfig} onReset={handleReset}/>
          ...
          <form onSubmit={handleSubmit} style={{ position:'relative', zIndex:1 }}>
            <div style={{ display:'flex', alignItems:'center', gap:'.5rem', borderBottom:`1px solid ${T.line}`, paddingBottom:'1rem', marginBottom:'1.75rem', animation:'slideRight .4s ease both' }}>
              ...
              <div>
                ...
                <div style={{ color:T.grey, fontSize:'.78rem', marginTop:'2px' }}>Applying for <strong style={{ color:T.orange }}>{jobConfig.position}</strong></div>
              </div>
            </div>

            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem', marginBottom:'1rem' }}>
              <Field icon={User} label="Full Name *" delay=".05s">
                <input type="text" required placeholder="John Doe" value={fullName} onChange={e=>setFullName(e.target.value)} style={inputStyle}
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem', marginBottom:'1.5rem' }}>
              <Field icon={Phone} label="Mobile Phone" delay=".15s">
                <input type="tel" placeholder="+92 300 1234567" value={mobile} onChange={e=>setMobile(e.target.value)} style={inputStyle}
                  onFocus={e=>{e.target.style.borderColor=T.orange;e.target.style.boxShadow='0 0 0 3px rgba(255,136,0,.12)';}}
                  onBlur={e=>{e.target.style.borderColor=T.line;e.target.style.boxShadow='none';}}/>
              </Field>
              <Field icon={MapPin} label="Location *" delay=".2s">
                <input type="text" required placeholder="Lahore, Pakistan" value={location} onChange={e=>setLocation(e.target.value)} style={inputStyle}
                  onFocus={e=>{e.target.style.borderColor=T.orange;e.target.style.boxShadow='0 0 0 3px rgba(255,136,0,.12)';}}
                  onBlur={e=>{e.target.style.borderColor=T.line;e.target.style.boxShadow='none';}}/>
              </Field>
            </div>
            ...
            <div style={{ animation:'fadeUp .5s ease .25s both' }}>
              ...
              <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}
                ...
                style={{ border:`2px dashed ${dragging?T.orange:file?T.lineO:T.line}`, borderRadius:'16px', padding:'2.5rem 2rem', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'.9rem', cursor:'pointer', textAlign:'center', background:dragging?'rgba(255,136,0,.04)':file?'rgba(255,136,0,.02)':'transparent', transition:'all .25s', ...(file&&!dragging?{animation:'borderPulse 2s ease-in-out infinite'}:{}) }}>
                ...
                {file ? (
                  <>
                    ...
                    <div>
                      ...
                      <div style={{ color:T.greyD, fontSize:'.8rem', marginTop:'.25rem' }}>
                        {parsing
                          ? <span style={{ color:T.orangeL, display:'flex', alignItems:'center', justifyContent:'center', gap:'.4rem' }}><Loader2 size={13} style={{ animation:'spin 1s linear infinite' }}/> Auto-extracting details…</span>
                  ...
                  <>
                    ...
                    <div>
                      <div style={{ color:T.white, fontWeight:600, fontSize:'1rem' }}>Drag & drop or <span style={{ color:T.orange, textDecoration:'underline' }}>browse</span></div>
            ...
            <div style={{ display:'flex', justifyContent:'flex-end', marginTop:'2rem', animation:'fadeUp .5s ease .3s both' }}>
              <button type="submit" className="cp-glow"
      ...
      {file && !result && !loading && <CVEnhancer file={file} activeConfig={jobConfig}/>}
    </div>
  );
}
